...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const res = await fetch(`https://ecommerce-nextjs-server.onrender.com/api/users/seller/getallproducts`, {
          method: 'GET',
        });
        if (!res.ok) {
  ...
  const handleCart = async(productId) => {
    ...
    try {
      const res = await fetch(`https://ecommerce-nextjs-server.onrender.com/api/users/buyer/addtocart`, {
        method: 'POST',
        headers: {
      ...
      setError('');
      setSuccess(true);
      alert('Congratulations!!. Product Successfully added to cart.');
    } catch (err) {
      setError(err.message);
    }
  }
        ...
        <div className="relative flex items-center justify-between w-full mb-2">
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 bg-black rounded-lg shadow-lg sm:hidden">
              ...
              <Link
                href="/buyer-dashboard/mycart"
                className="block text-yellow-300 px-4 py-2 hover:bg-gray-800"
